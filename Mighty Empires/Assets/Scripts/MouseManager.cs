...
public class MouseManager : MonoBehaviour {
	...
	void Update () {
		...
		if(EventSystem.current.IsPointerOverGameObject())
		{
			...
			// mouse stuff, because that would be wierd.
		...
		// If this were an FPS, you'd probably do something like this:
	...
	{
		Debug.Log("Raycast hit: " + ourHitObject.name); //hitInfo.collider.transform.parent.gameObject.name);

		// We know what we're mousing over, maybe we want to show a tooltip?

		// Do we have a unit selected? Because that might change what we do on click.
		...
		if( Input.GetMouseButtonDown(0))
		{
			...
			// We're just gonna colourise the hex, as an example.
